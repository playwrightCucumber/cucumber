...
export class ROIPage {
  ...
  async editActivityNote(oldText: string, newText: string): Promise<void> {
    ...
    try {
      ...
      // Find the note by text
      const noteElement = this.page.getByText(oldText, { exact: false }).first();
      await noteElement.waitFor({ state: 'visible', timeout: 5000 });

      // Click three dots menu (already correct)
      const threeDotsMenu = this.page
        .locator(RoiSelectors.activityNoteThreeDotsMenu)
        .first();
      ...
      // ⬇️ CRITICAL FIX: anchor from the SAME activity as the menu
      const activityContainer = this.page.locator(
        '[data-testid="user-log-activity-div-content-activity-0"]'
      );

      // Target ONLY the inline edit textarea
      const targetTextarea = activityContainer.locator(
        '[data-testid="user-log-activity-div-edit-activity"] textarea[data-testid="user-log-activity-textarea"]'
      );

      await targetTextarea.waitFor({ state: 'visible', timeout: 5000 });

      // Focus first (Angular Material requirement)
      await targetTextarea.click();
      await targetTextarea.fill(newText);

      // Click save (checkmark) scoped to same activity
      const saveButton = activityContainer.locator(
        '[data-testid="user-log-activity-div-group-button-edit"] mat-icon[data-mat-icon-name="check-edit"]'
      );
      ...
      try {
        ...
        this.logger.warn('Checkmark still visible after 3s');
      }
